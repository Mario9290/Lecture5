...
int Sum(std::vector<int> nums)
{
    ...
    // Loop through each element in the vector and add it to the total
    for (int num : nums) {
        total += num;
    }
...
int Sign(int num)
{
	if (num < 0) 
	{
	 ...
	 // return 1 if the number is positive or zero
    return 1; 
}
...
double Sum(std::vector<double> nums)
{
    ...
    // Iterate through each element in the vector and add it to total
    for (double num : nums) 
	{
